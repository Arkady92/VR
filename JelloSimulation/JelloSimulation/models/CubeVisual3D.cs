...
namespace JelloSimulation.Models
{
    public class BezierCubeVisual3D : BaseVisual3D, IBezierCubeVisual3D
    {
        ...
        private List<Point3D> sphereBezierParameters;
            ...
            for (double u = 0; u <= 1; u += step)
            {
                ...
                for (double v = 0; v <= 1; v += step)
                {
                    ...
                    for (double w = 0; w <= 1; w += step)
                    {
                        ...
                        if (Math.Abs(Math.Sqrt(sum.X * sum.X + sum.Y * sum.Y + sum.Z * sum.Z) - sphereRadius) < 0.01)
                        {
        ...
        private void randomizeInitialPositions()
        {
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    for (int k = 0; k < N; k++)
                    {
                        X[i, j, k].X += X0Max * (rand.NextDouble() - 0.5) * 2;
                        X[i, j, k].Y += X0Max * (rand.NextDouble() - 0.5) * 2;
                        X[i, j, k].Z += X0Max * (rand.NextDouble() - 0.5) * 2;
                    }
                }
            }
        }
        ...
        private void randomizeInitialVelocities()
        {
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    for (int k = 0; k < N; k++)
                    {
                        X[i, j, k].X += V0Max * (rand.NextDouble() - 0.5) * 2;
                        X[i, j, k].Y += V0Max * (rand.NextDouble() - 0.5) * 2;
                        X[i, j, k].Z += V0Max * (rand.NextDouble() - 0.5) * 2;
                    }
                }
            }
        }
